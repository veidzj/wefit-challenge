...
describe('AddNaturalPersonMySQLRepository', () => {
  ...
  test('Should add a natural person on success', async() => {
    ...
    expect(rows[0].cep).toBe(addNaturalPersonRepositoryInput.address.cep)
    expect(rows[0].street).toBe(addNaturalPersonRepositoryInput.address.street)
    expect(rows[0].number).toBe(addNaturalPersonRepositoryInput.address.number)
    expect(rows[0].complement).toBe(addNaturalPersonRepositoryInput.address.complement)
    expect(rows[0].city).toBe(addNaturalPersonRepositoryInput.address.city)
    expect(rows[0].neighborhood).toBe(addNaturalPersonRepositoryInput.address.neighborhood)
    expect(rows[0].state).toBe(addNaturalPersonRepositoryInput.address.state)
  })
})
